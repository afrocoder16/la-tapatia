...
This folder is the selected client handoff sample. It is a static website with a front-end cart, no build step, no npm install, and no backend.
...
- Business phones, social links, cart order email, and demo location: `js/config.js`
...
- The contact form and cart order request open the visitor's email app with a `mailto:` link. Add a form service or backend later if the client wants direct form submission.
- The cart is ready for Square checkout wiring, but Square POS/API credentials are not connected yet.
